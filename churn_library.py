...
def perform_feature_engineering(data_frame, response='Churn'):
    ...
    data_frame = encode_categorical_variables(data_frame, CAT_COLUMNS, response)
...
def evaluate_models(lrc_model, rfc_model,
    ...
    feature_importance_plot(model=rfc_model, feature_data=x_train, output_path=f"./{CONFIG['paths']['results']}")
...
def train_models(x_train, y_train):
    ...
    # Define the list of models and their parameter spaces for hyperparameter tuning
    models_params = [
        {
    ...
    # Loop through the list of models to perform randomized search and save the best model
